...
### v2.2.1 (2026-06-10)
- 🎨 **玩法标签颜色差异化显示**
  - 服务器列表卡片与精选大图卡片的玩法标签根据类型自动显示不同颜色
  - 支持大小写不敏感匹配、部分匹配和哈希颜色分配
  - 覆盖 18+ 种常见玩法：生存(绿)/生电(黄)/建筑(蓝)/RPG(红)/空岛(青)/小游戏(橙)/PVP(粉)/模组(紫)/原版(灰)/红石(橙)/冒险(蓝绿)/角色(紫)/创意(蓝)等
  - 未知标签基于内容哈希自动分配颜色，避免全部显示默认色

### v2.1.0
- 🎉 **版本号升级至 2.1.0**
- ✨ 新增首页精选服务器大图推荐组件（支持随机/热门/最新/最新加入四种推荐模式）
- 🔧 重构服务器列表模块，新增开关控制与排序配置（热度/随机/最新）
- 📄 完善服务器详情页，支持 QQ群 与官网信息展示
- ⚙️ 新增设置页面，支持自定义插件功能开关与排序方式配置
- 🔧 优化 HTML 文本处理逻辑与图标样式替换
- 🐛 修复部分文案与显示问题
